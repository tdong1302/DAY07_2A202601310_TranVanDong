...
class EmbeddingStore:
    ...
    def add_documents(self, docs: list[Document]) -> None:
        ...
        if self._use_chroma and self._collection is not None and records:
            self._collection.add(
                ids=[record["store_id"] for record in records],
    ...
    def delete_document(self, doc_id: str) -> bool:
        ...
        matching_store_ids = {record["store_id"] for record in matching}
        self._store = [record for record in self._store if record["store_id"] not in matching_store_ids]

        if self._use_chroma and self._collection is not None:
            self._collection.delete(ids=sorted(matching_store_ids))
        return True
